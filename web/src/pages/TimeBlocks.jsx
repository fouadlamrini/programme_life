...
const isAutoActivity = (activity) => {
  if (!activity) return false
  const title = String(activity.title || '').trim().toLowerCase()
  if (activity.type === 'ADHKAR' && (title.includes('الصباح') || title.includes('صباح'))) return true
  if (activity.type === 'ADHKAR' && (title.includes('المساء') || title.includes('مساء'))) return true
  if (activity.type === 'QURAN' && title.includes('قراءة القرآن')) return true
  if (activity.type === 'QURAN' && (title.includes('الكهف') || title.includes('كهف'))) return true
  return false
}

const isAutoBlock = (block) => Boolean(block && block.autoGenerated)

const isLockedBlock = (block, activitiesList) => isPrayerBlock(block, activitiesList) || isAutoBlock(block)
                ...
                <li key={block._id} className="flex flex-col justify-between gap-3 rounded-2xl border border-[#e5dbc4] bg-[#fbf8ef] p-4">
                  <div>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex shrink-0 gap-1">
                        ...
                        {isAutoBlock(block) && (
                          <span className="rounded-full bg-[#fff0d9] px-3 py-1 text-xs font-semibold text-[#b8731b]">
                            تلقائي
                          </span>
                        )}
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex gap-2">
                      {!isLockedBlock(block, activities) && (
                        <>
                          <button
            ...
            <form className="mt-6 flex flex-col gap-4" onSubmit={handleSubmit}>
                ...
                <label className="flex flex-col gap-1">
                  ...
                  >
                    ...
                    {activities.filter((a) => !isPrayerActivity(a) && !isAutoActivity(a)).map((activity) => (
